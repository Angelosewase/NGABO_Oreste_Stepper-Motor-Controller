...
# Try Media Foundation first (better for modern Windows cameras)
print("Attempting to open camera with Media Foundation...")
cap = cv2.VideoCapture(0, cv2.CAP_MSMF)  # Media Foundation - better for Windows 10/11

if not cap.isOpened():
    print("Media Foundation failed, trying DirectShow...")
    cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # DirectShow fallback
...
# Camera adjustments for better visibility (especially in low light)
...
# Add delay for camera to initialize
import time as camera_init_time
camera_init_time.sleep(1)
...
# Warm up camera - discard first few frames (often dark/black)
print("Warming up camera, please wait...")
for i in range(10):
    ret, _ = cap.read()
    if not ret:
        print(f"Warning: Failed to read warm-up frame {i+1}")
print("Camera ready!")
...
while True:
    ...
    # Check if frame is completely black (camera issue)
    if np.max(frame) == 0:
        cv2.putText(frame, "CAMERA ERROR: Black frame detected", (10, 30),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(frame, "Try: Close other apps, check camera privacy settings", (10, 60),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
    
    # Auto-boost very dark frames
    frame_brightness = np.mean(frame)
    if 0 < frame_brightness < 30:
        frame = cv2.convertScaleAbs(frame, alpha=2.5, beta=50)
